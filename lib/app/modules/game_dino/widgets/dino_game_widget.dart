...
class DinoGameWidget extends StatefulWidget {
  final Function(int score, int highScore)? onScoreUpdate;
  final Function(DinoState state)? onDinoStateChange;

  const DinoGameWidget({
    super.key,
    this.onScoreUpdate,
    this.onDinoStateChange,
  });
...
class _DinoGameWidgetState extends State<DinoGameWidget>
  ...
  void initState() {
    super.initState();
    dino = Dino(onStateChange: widget.onDinoStateChange);
    worldController =
  ...
  void _newGame() {
    setState(() {
      ...
      widget.onDinoStateChange?.call(dino.state);
      dino.dispY = 0;
          ...
          child: Stack(
            ...
            children: [
              ...
              AnimatedBuilder(
                animation: worldController,
                builder: (context, _) {
                  widget.onScoreUpdate?.call(runDistance.toInt(), highScore);
                  return Positioned(
